...
describe("decode", () => {
  test("MainsCurrentL1 deci-A → A", () => {
    expect(decode("MainsCurrentL1", "105")).toEqual({
      capId: "measure_current.l1",
      value: 10.5,
    });
  });

  test("MainsCurrentL2 negative (exporting)", () => {
    expect(decode("MainsCurrentL2", "-3")).toEqual({
      capId: "measure_current.l2",
      value: -0.3,
    });
  });

  test("EVCurrentL3 deci-A → A", () => {
    expect(decode("EVCurrentL3", "160")).toEqual({
      capId: "measure_current.ev_l3",
      value: 16.0,
    });
  });
  ...
  test("MainsImportActiveEnergy Wh → kWh", () => {
    expect(decode("MainsImportActiveEnergy", "12345")).toEqual({
      capId: "meter_power.mains_import",
      value: 12.345,
    });
  });

  test("EVChargePower raw W", () => {
    expect(decode("EVChargePower", "6900")).toEqual({
      capId: "measure_power",
      value: 6900,
    });
  });
  ...
  test("EVEnergyCharged Wh → kWh", () => {
    expect(decode("EVEnergyCharged", "500")).toEqual({
      capId: "meter_power.charged",
      value: 0.5,
    });
  });
  ...
  test("MaxCurrent deci-A ×10 (firmware publishes MaxCurrent*10) → A", () => {
    ...
    expect(decode("MaxCurrent", "160")).toEqual({
      capId: "max_current",
      value: 16,
    });
  });
  ...
  test("ChargeCurrentOverride deci-A → A", () => {
    expect(decode("ChargeCurrentOverride", "100")).toEqual({
      capId: "charge_current",
      value: 10,
    });
  });

  test("EVPlugState", () => {
    expect(decode("EVPlugState", "Connected")).toEqual({
      capId: "ev_plug_state",
      value: "Connected",
    });
  });
  ...
  test("State passthrough text", () => {
    expect(decode("State", "State C, Charging")).toEqual({
      capId: "charger_state",
      value: "State C, Charging",
    });
  });

  test("LEDColorOff RGB parse", () => {
    expect(decode("LEDColorOff", "0,255,0")).toEqual({
      capId: "led_color_off",
      value: "0,255,0",
    });
  });
  ...
  test("WiFiRSSI dBm", () => {
    expect(decode("WiFiRSSI", "-62")).toEqual({
      capId: "measure_rssi",
      value: -62,
    });
  });
  ...
  test("connected → online=true", () => {
    expect(decode("connected", "online")).toEqual({
      capId: "online",
      value: true,
    });
  });
  ...
  test("connected → online=false", () => {
    expect(decode("connected", "offline")).toEqual({
      capId: "online",
      value: false,
    });
  });
  ...
  test("NrOfPhases → string enum", () => {
    expect(decode("NrOfPhases", "3")).toEqual({
      capId: "nr_of_phases",
      value: "3",
    });
  });
...
const DECODERS: Record<string, Decoder> = {
  ...
  NrOfPhases: (p) =>
    p === "1" || p === "3" ? { capId: "nr_of_phases", value: p } : null,
  CPPWM: num("cp_pwm", 1),
...
describe("encode", () => {
  test("mode capability → Set/Mode payload", () => {
    expect(encode("mode", "Smart")).toEqual({
      topic: "Set/Mode",
      payload: "Smart",
    });
  });
  ...
  test("charge_current A → Set/CurrentOverride deci-A", () => {
    expect(encode("charge_current", 16)).toEqual({
      topic: "Set/CurrentOverride",
      payload: "160",
    });
  });

  test("charge_current rounds to nearest deci-A", () => {
    expect(encode("charge_current", 6.04)).toEqual({
      topic: "Set/CurrentOverride",
      payload: "60",
    });
  });
  ...
  test("cable_lock numeric", () => {
    expect(encode("cable_lock", 2)).toEqual({
      topic: "Set/CableLock",
      payload: "2",
    });
  });
  ...
  test("enable_c2 string", () => {
    expect(encode("enable_c2", "Auto")).toEqual({
      topic: "Set/EnableC2",
      payload: "Auto",
    });
  });
  ...
  test("required_evccid", () => {
    expect(encode("required_evccid", "ABCD1234")).toEqual({
      topic: "Set/RequiredEVCCID",
      payload: "ABCD1234",
    });
  });

  test("led_color_normal RGB", () => {
    expect(encode("led_color_normal", "0,255,0")).toEqual({
      topic: "Set/ColorNormal",
      payload: "0,255,0",
    });
  });
  ...
  test("max_sum_mains A integer (clamped 10-600)", () => {
    expect(encode("max_sum_mains", 300)).toEqual({
      topic: "Set/CurrentMaxSumMains",
      payload: "300",
    });
  });
  ...
  test("cp_pwm_override numeric passthrough", () => {
    expect(encode("cp_pwm_override", 50)).toEqual({
      topic: "Set/CPPWMOverride",
      payload: "50",
    });
  });
...
const ENCODERS: Record<string, Encoder> = {
  ...
  charge_current: (v) => {
    ...
    return {
      topic: "Set/CurrentOverride",
      payload: String(Math.round(v * 10)),
    };
  },
  cable_lock: (v) => {
  ...
  required_evccid: (v) =>
    typeof v === "string" ? { topic: "Set/RequiredEVCCID", payload: v } : null,
  ...
  cp_pwm_override: (v) =>
    typeof v === "number" && Number.isFinite(v)
      ? { topic: "Set/CPPWMOverride", payload: String(Math.round(v)) }
      : null,
  custom_button: (v) =>
    v === "On" || v === "Off" ? { topic: "CustomButton", payload: v } : null,
};
...
describe("encodeEvMeter", () => {
  test("five-field payload", () => {
    expect(encodeEvMeter(10, 10, 10, 6900, 12345)).toBe(
      "100:100:100:6900:12345",
    );
  });
});
...
export function encodeRfidSwipe(uidHex: string): string {
  if (!HEX_RE.test(uidHex) || (uidHex.length !== 12 && uidHex.length !== 14)) {
    throw new Error(
      `Invalid RFID UID: expected 12 or 14 hex characters, got "${uidHex}"`,
    );
  }
...
describe("deriveChargingState", () => {
  test.each([
    ...
    [
      { plug: "Connected", state: "State C, Charging", mode: "Normal" },
      "plugged_in_charging",
    ],
    [
      { plug: "Connected", state: "State B", mode: "Pause" },
      "plugged_in_paused",
    ],
...
describe("deriveIsCharging", () => {
  ...
  test("undefined → false", () =>
    expect(deriveIsCharging(undefined)).toBe(false));
});
...
export function deriveChargingState({
  ...
  state,
  mode,
}: ChargingStateInputs): ChargingState {
...
export function deriveTargetPower(
  amps: number | undefined,
  phases: "1" | "3" | undefined,
): number {
...
export const ALL_CAPS: readonly string[] = [
  ...CAPABILITY_MAP.nativeCaps,
  ...CAPABILITY_MAP.customCaps,
];
```
...
describe("MqttHub routing", () => {
  test("subscribe registers prefix and routes messages to handler", async () => {
    ...
    expect(c.subscribe).toHaveBeenCalledWith(
      "SmartEVSE/1234/#",
      { qos: 0 },
      expect.any(Function),
    );
  ...
  test("overlapping prefixes use longest match", async () => {
    ...
    hub.subscribe({
      prefix: "SmartEVSE/1234",
      onMessage: (s) => shortMsgs.push(s),
      onOnline: () => {},
    });
    hub.subscribe({
      prefix: "SmartEVSE/1234-test",
      onMessage: (s) => longMsgs.push(s),
      onOnline: () => {},
    });
  ...
  test("offline event broadcasts onOnline(false) to every subscription", async () => {
    ...
    hub.subscribe({
      prefix: "a",
      onMessage: () => {},
      onOnline: (o) => aOnline.push(o),
    });
    hub.subscribe({
      prefix: "b",
      onMessage: () => {},
      onOnline: (o) => bOnline.push(o),
    });
    c.connected = false;
    c.emit("offline");
...
```json
{
  ...
  "values": [
    ...
    {
      "id": "Disconnected",
      "title": { "en": "Disconnected", "nl": "Niet aangesloten" }
    }
  ]
}
```
...
```json
{
  ...
  "values": [
    {
      "id": "Not present",
      "title": { "en": "Not present", "nl": "Niet aanwezig" }
    },
    { "id": "Always Off", "title": { "en": "Always off", "nl": "Altijd uit" } },
    {
      "id": "Solar Off",
      "title": { "en": "Off on solar", "nl": "Uit bij zon" }
    },
...
```json
{
  ...
  "pair": {
    ...
    "name": "Device name",
    "prefix": "Topic prefix",
    "prefix_hint": "e.g. SmartEVSE/1234",
...
```json
{
  ...
  "pair": {
    ...
    "name": "Apparaatnaam",
    "prefix": "Topic-prefix",
    "prefix_hint": "bijv. SmartEVSE/1234",
...
<fieldset class="homey-form-fieldset">
  <div class="homey-form-group">
    <label class="homey-form-label" for="name" data-i18n="pair.name"
      >Device name</label
    >
    <input id="name" type="text" class="homey-form-input" value="SmartEVSE" />
  </div>
  <div class="homey-form-group">
    <label class="homey-form-label" for="prefix" data-i18n="pair.prefix"
      >Topic prefix</label
    >
    <input
      id="prefix"
...
module.exports = class SmartEvseDriver extends Homey.Driver {
  ...
  private registerFlowActions(): void {
    ...
    feedMains.registerRunListener(
      async (args: { device: any; l1: number; l2: number; l3: number }) => {
        await args.device.publish(
          "Set/MainsMeter",
          encodeMainsMeter(args.l1, args.l2, args.l3),
        );
      },
    );

    const feedEv = this.homey.flow.getActionCard("feed_ev_meter");
    feedEv.registerRunListener(
      async (args: {
        device: any;
        l1: number;
        l2: number;
        ...
        await args.device.publish(
          "Set/EVMeter",
          encodeEvMeter(args.l1, args.l2, args.l3, args.power, args.energy),
        );
      },
    );

    const feedBattery = this.homey.flow.getActionCard(
      "feed_home_battery_current",
    );
    feedBattery.registerRunListener(
      async (args: { device: any; amps: number }) => {
        await args.device.publish(
          "Set/HomeBatteryCurrent",
          encodeHomeBatteryCurrent(args.amps),
        );
      },
    );
...
```json
{
  "id": "feed_mains_meter",
  "title": {
    "en": "Feed mains meter (L1/L2/L3)",
    "nl": "Netmeter voeden (L1/L2/L3)"
  },
  "titleFormatted": {
  ...
  "args": [
    { "name": "device", "type": "device", "filter": "driver_id=smartevse" },
    {
      ...
      "max": 600,
      "step": 0.1,
      "placeholder": { "en": "L1 amps", "nl": "L1 ampère" }
    },
    {
      "name": "l2",
      "type": "number",
      ...
      "max": 600,
      "step": 0.1,
      "placeholder": { "en": "L2 amps", "nl": "L2 ampère" }
    },
    {
      "name": "l3",
      "type": "number",
      ...
      "max": 600,
      "step": 0.1,
      "placeholder": { "en": "L3 amps", "nl": "L3 ampère" }
    }
  ]
}
```
...
```json
{
  "id": "feed_ev_meter",
  "title": {
    "en": "Feed EV meter (L1/L2/L3/P/E)",
    "nl": "EV-meter voeden (L1/L2/L3/P/E)"
  },
  "titleFormatted": {
  ...
  "args": [
    ...
    { "name": "l3", "type": "number", "min": -600, "max": 600, "step": 0.1 },
    {
      ...
      "max": 200000,
      "step": 1,
      "placeholder": { "en": "Power (W)", "nl": "Vermogen (W)" }
    },
    {
      "name": "energy",
      "type": "number",
      ...
      "max": 1000000000,
      "step": 1,
      "placeholder": { "en": "Total energy (Wh)", "nl": "Totale energie (Wh)" }
    }
  ]
}
```
...
```json
{
  ...
  "title": {
    "en": "Feed home battery current",
    "nl": "Thuisaccustroom voeden"
  },
  "titleFormatted": {
...
```json
{
  ...
  "args": [
    { "name": "device", "type": "device", "filter": "driver_id=smartevse" },
    {
      "name": "uid",
      "type": "text",
      "placeholder": { "en": "12 or 14 hex chars", "nl": "12 of 14 hex-tekens" }
    }
  ]
}
```
...
```json
{
  ...
  "title": {
    "en": "Set charge current override",
    "nl": "Laadstroom-override instellen"
  },
  "titleFormatted": {
...
```json
{
  "id": "set_enable_c2",
  "title": {
    "en": "Set phase contactor (C2)",
    "nl": "Fasecontactor (C2) instellen"
  },
  "titleFormatted": {
  ...
  "args": [
    { "name": "device", "type": "device", "filter": "driver_id=smartevse" },
    {
      ...
      "values": [
        {
          "id": "Not present",
          "label": { "en": "Not present", "nl": "Niet aanwezig" }
        },
        {
          "id": "Always Off",
          "label": { "en": "Always off", "nl": "Altijd uit" }
        },
        {
          "id": "Solar Off",
          "label": { "en": "Off on solar", "nl": "Uit bij zon" }
        },
        {
          "id": "Always On",
          "label": { "en": "Always on", "nl": "Altijd aan" }
        },
...
setLed.registerRunListener(
  async (args: {
    ...
    await args.device.setCapabilityValue(
      capId,
  ...
  "tokens": [
    {
      "name": "state",
      "type": "string",
      "title": { "en": "State", "nl": "Status" },
      "example": { "en": "State C, Charging" }
    }
  ],
  "args": [
    { "name": "device", "type": "device", "filter": "driver_id=smartevse" }
...
```json
{
  "id": "is_plugged_in",
  "title": {
    "en": "EV !{{is|isn't}} plugged in",
    "nl": "EV !{{is|is niet}} aangesloten"
  },
  "args": [
    { "name": "device", "type": "device", "filter": "driver_id=smartevse" }
  ]
...
```json
{
  "id": "has_error",
  "title": {
    "en": "Has !{{an error|no error}}",
    "nl": "Heeft !{{een fout|geen fout}}"
  },
  "args": [
    { "name": "device", "type": "device", "filter": "driver_id=smartevse" }
  ]
...
module.exports = class SmartEvseDevice extends Homey.Device {
  ...
  private onTopic(suffix: string, payload: string): void {
    ...
    if (suffix === "Error")
      this.tryTrigger("error_changed", { error: res.value });
    if (suffix === "State")
      this.tryTrigger("charger_state_changed", { state: res.value });
    if (suffix === "RFIDLastRead")
      this.tryTrigger("rfid_swiped", { uid: res.value });
  ...
  private onOnlineChange(online: boolean): void {
    ...
    if (online) this.setAvailable().catch(() => {});
      ...
      this.setUnavailable(this.homey.__("unavailable.device_offline")).catch(
        () => {},
      );
  }
...
<html>
  ...
  <body>
    ...
    <fieldset class="homey-form-fieldset">
      ...
      <div class="homey-form-group">
        <label class="homey-form-label" data-i18n="settings.protocol"
          >Protocol</label
        >
      ...
      <div class="homey-form-group">
        <label class="homey-form-label" data-i18n="settings.username"
          >Username</label
        >
        <input id="username" type="text" class="homey-form-input" />
      </div>
      <div class="homey-form-group">
        <label class="homey-form-label" data-i18n="settings.password"
          >Password</label
        >
        ...
        document.getElementById("save").addEventListener("click", () => {
          ...
          Homey.set("mqtt", cfg, (err) => {
            document.getElementById("status").textContent = err
              ? String(err)
              : Homey.__("settings.status_connected");
          });
        });
      }
    </script>
